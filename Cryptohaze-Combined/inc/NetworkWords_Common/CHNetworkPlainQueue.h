...
// The max length of plains we support.
// Longer data is not accepted.
#define MAX_PLAIN_LENGTH 32

// Max number of IO threads
#define MAX_IO_THREADS 5

// Data comes and goes in vectors of char[MAX_PLAIN_LENGTH] arrays.
...
public:
    ...
    void addPlainsToQueue(std::vector <std::string>);
    
    // Returns the requested number of plains, or the maximum present.
    std::vector <std::string> getNextNPlains(uint32_t);
...
private:
    ...
    std::queue <std::string> plainQueue;
