...
int GRTCLGenCommandLineData::ParseCommandLine(int argc, char *argv[]) {
    ...
    struct arg_str *hash_type = arg_str1("h", "hashtype", "{NTLM, MD4, MD5, SHA1}", "hash type to create tables for");
    struct arg_file *charset_file = arg_file1("c", "charsetfile", "charsetfile", "charset file");
    struct arg_int *pass_len = arg_int1("l", "passwordlen", "<n>", "password length to use");
    struct arg_int *table_index = arg_int1("i", "tableindex", "<n>", "table index");
    struct arg_int *chain_length = arg_int1(NULL, "chainlength", "<n>", "length of each chain");
    ...
    void *argtable[] = {verbose,hash_type,charset_file,pass_len,table_index,
        ...
        table_version,amd_kernels,vector_width,table_bits,end};
