...
int main(int argc, char *argv[]) {
    ...
    switch(CommandLineData.getHashType()) {
        case 0:
            CandidateHash = new GRTCLCandidateHashesNTLM();
            RegenChains = new GRTCLRegenerateChainsNTLM();
            HashFile = new GRTHashFilePlain(16);
            break;
        case 1:
            CandidateHash = new GRTCLCandidateHashesMD5();
            RegenChains = new GRTCLRegenerateChainsMD5();
            HashFile = new GRTHashFilePlain(16);
            break;
        case 3:
            CandidateHash = new GRTCLCandidateHashesSHA1();
            RegenChains = new GRTCLRegenerateChainsSHA1();
    ...
    for (i = CommandLineData.getOpenCLDevice(); i < (CommandLineData.getOpenCLDevice() + 1) /*CommandLineData.getCudaNumberDevices()*/; i++) {
    ...
    for (i = 0; i < CommandLineData.getNumberOfTableFiles(); i++) {
        ...
        // Add chains to match the vector width
        while (chainsToRegen->size() % CommandLineData.getVectorWidth()) {
            hashPasswordData vectorLengthPadding;

            memset(&vectorLengthPadding, 0, sizeof(hashPasswordData));
            chainsToRegen->push_back(vectorLengthPadding);
            //printf("added vector padding\n");
        }
