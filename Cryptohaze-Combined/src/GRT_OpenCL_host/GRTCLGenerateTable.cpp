...
void GRTCLGenerateTable::generateInitialPasswords(int passwordType) {
    ...
    memset(this->HOST_Initial_Passwords, 0, this->PasswordInputBlockLengthBytes * this->TableParameters->getNumberChains() * sizeof(unsigned char));

    for (chain_index = 0; chain_index < this->TableParameters->getNumberChains(); chain_index++) {
        // Run through the password length.
        for (password_position = 0; password_position < this->TableParameters->getPasswordLength(); password_position++) {
            ...
            character_offset = (chain_index * 4) // Initial offset based on the chain index.
                    ...
                    + (password_position / 4) * (4 * this->TableParameters->getNumberChains());  // The offset of the chunk
...
void GRTCLGenerateTable::mallocDeviceAndHostSpace() {
    ...
    if (this->TableParameters->getNumberChains() == 0) {
    ...
    printf("Using %lu chains in GPU RAM\n", this->TableParameters->getNumberChains());
        ...
        numberChainsToTry = this->TableParameters->getNumberChains();
        ...
        this->DEVICE_Initial_Passwords = clCreateBuffer (this->OpenCL->getContext(),
            CL_MEM_READ_WRITE,
            this->PasswordInputBlockLengthBytes * this->TableParameters->getNumberChains() * sizeof(unsigned char),
            NULL,
            &errorCode);
        ...
        this->DEVICE_End_Hashes = clCreateBuffer (this->OpenCL->getContext(),
            CL_MEM_WRITE_ONLY,
            this->HashOutputBlockLengthBytes * this->TableParameters->getNumberChains() * sizeof(unsigned char),
            NULL,
            &errorCode);
    ...
    this->HOST_Initial_Passwords = (unsigned char*) malloc(this->PasswordInputBlockLengthBytes * this->TableParameters->getNumberChains() * sizeof(unsigned char));
    this->HOST_End_Hashes = (unsigned char*) malloc(this->HashOutputBlockLengthBytes * this->TableParameters->getNumberChains() * sizeof(unsigned char));
...
void GRTCLGenerateTable::createTables() {
    ...
    sprintf(buildOptions, "-D PASSWORD_LENGTH=%d -I .  -I ./kernels", this->TableParameters->getPasswordLength());
    ...
    if (this->TableParameters->getUseAmdKernels()) {
        ...
        this->OpenCL->buildProgramFromManySourcesConcat(this->getHashFileName(), buildOptions);
    ...
    deviceChainLength = this->TableParameters->getChainLength();
    deviceNumberOfChains = this->TableParameters->getNumberChains();
    deviceTableIndex = this->TableParameters->getTableIndex();
...
*/
    ...
    for (tableNumber = 0; tableNumber < this->TableParameters->getNumberTables(); tableNumber++) {
        ...
        errorCode = clEnqueueWriteBuffer (GenerateTableCommandQueue,
            ...
            CL_TRUE /* blocking write */,
            0 /* offset */,
            this->PasswordInputBlockLengthBytes * this->TableParameters->getNumberChains() * sizeof(unsigned char) /* bytes to copy */,
        ...
        if (!this->TableParameters->getKernelTimeMs()) {
            StepsPerInvocation = this->TableParameters->getChainLength();
        }

        NumberOfChainsToRun = this->TableParameters->getNumberChains();
        ...
        while (ChainsCompleted < NumberOfChainsToRun) {
            ...
            while (CurrentChainStartOffset < this->TableParameters->getChainLength()) {
                ...
                if ((CurrentChainStartOffset + StepsPerInvocation) > this->TableParameters->getChainLength()) {
                    StepsPerInvocation = this->TableParameters->getChainLength() - CurrentChainStartOffset;
                }
                ...
                if ((NumberOfChainsToRun - ChainsCompleted) < ActiveThreads) {
                    ActiveThreads = (/*this->TableParameters->getNumberChains()*/NumberOfChainsToRun - ChainsCompleted);
                ...
                printf("Kernel Time: %0.03f ms  Step rate: %0.2f M/s Done: %0.2f%%  \r",
                    ...
                    (float)(100 * ((float)((float)ChainsCompleted * (float)this->TableParameters->getChainLength() +
                        (float)CurrentChainStartOffset * (float)ActiveThreads) /
                        (float)((float)NumberOfChainsToRun * (float)this->TableParameters->getChainLength()))));
                fflush(stdout);
                ...
                if (this->TableParameters->getKernelTimeMs()) {
                    ...
                    if ((ref_time > 1.1 * this->TableParameters->getKernelTimeMs()) || (ref_time < 0.9 * this->TableParameters->getKernelTimeMs())) {
						...
						//printf("\nKernel time goal: %d\n",this->TableParameters->getKernelTimeMs()); 
        ...
        printf("\n\nDone chain running\n");
        printf("Total execution rate: %0.2f M/s\n", (float)((uint64_t)this->TableParameters->getNumberChains() *
                (uint64_t)this->TableParameters->getChainLength()) / (fullRunTimer.stop() * 1000000));

        errorCode = clEnqueueReadBuffer (GenerateTableCommandQueue,
            this->DEVICE_End_Hashes,
            CL_TRUE /* blocking write */,
            0 /* offset */,
            this->HashOutputBlockLengthBytes * this->TableParameters->getNumberChains() * sizeof(unsigned char) /* bytes to copy */,
        ...
        if (0) {
            for (i = 0; i < this->TableParameters->getNumberChains(); i++) {
                UINT4 base_offset;

                // Print the end hashes
                printf("Chain %d: ", i);
                // Print the password
                base_offset = this->TableParameters->getNumberChains() * 0; // Segment start
                ...
                base_offset = this->TableParameters->getNumberChains() * 1 * 4; // Segment start
                base_offset += i * 4; // This chain start
                printf("%c%c%c%c  ", HOST_Initial_Passwords[base_offset], HOST_Initial_Passwords[base_offset + 1], HOST_Initial_Passwords[base_offset + 2], HOST_Initial_Passwords[base_offset + 3]);
                ...
                base_offset += i * 4; // This chain start
                printf("%02X%02X%02X%02X", HOST_End_Hashes[base_offset], HOST_End_Hashes[base_offset + 1], HOST_End_Hashes[base_offset + 2], HOST_End_Hashes[base_offset + 3]);
                base_offset = this->TableParameters->getNumberChains() * 2 * 4; // Segment start
                base_offset += i * 4; // This chain start
                printf("%02X%02X%02X%02X", HOST_End_Hashes[base_offset], HOST_End_Hashes[base_offset + 1], HOST_End_Hashes[base_offset + 2], HOST_End_Hashes[base_offset + 3]);
                base_offset = this->TableParameters->getNumberChains() * 3 * 4; // Segment start
                base_offset += i * 4; // This chain start
                printf("%02X%02X%02X%02X", HOST_End_Hashes[base_offset], HOST_End_Hashes[base_offset + 1], HOST_End_Hashes[base_offset + 2], HOST_End_Hashes[base_offset + 3]);
                printf("\n");
            }
        ...
        for (i = 0; i < this->TableParameters->getNumberChains(); i++) {
            ...
            for (j = 0; j < this->HashOutputBlockLengthBytes; j++) {
                array_offset = (i * 4) // Initial offset based on the chain index.
                  ...
                  + (j / 4) * (4 * this->TableParameters->getNumberChains());  // The offset of the chunk
        ...
        for (uint64_t password = 0; password < this->TableParameters->getNumberChains(); password++) {
        ...
        chainStartOffset += this->TableParameters->getNumberChains();
