...
__device__ __constant__ unsigned char MD5_Regenerate_Device_Charset_Constant[512]; // Constant space for charset
__device__ __constant__ uint32_t MD5_Regenerate_Device_Charset_Length; // Character set length
__device__ __constant__ uint32_t MD5_Regenerate_Device_Chain_Length; // May as well pull it from constant memory... faster when cached.
__device__ __constant__ uint32_t MD5_Regenerate_Device_Table_Index;
__device__ __constant__ uint32_t MD5_Regenerate_Device_Number_Of_Threads; // It needs this, and can't easily calculate it
__device__ __constant__ uint32_t MD5_Regenerate_Device_Number_Of_Chains_To_Regen;
__device__ __constant__ uint32_t MD5_Regenerate_Device_Number_Of_Hashes;
...
extern "C" void copyMD5RegenerateDataToConstant(char *hostCharset, uint32_t hostCharsetLength,
    ...
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(MD5_Regenerate_Device_Charset_Constant, hostCharset, 512));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(MD5_Regenerate_Device_Charset_Length, &hostCharsetLength, sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(MD5_Regenerate_Device_Chain_Length, &hostChainLength, sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(MD5_Regenerate_Device_Table_Index, &hostTableIndex, sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(MD5_Regenerate_Device_Number_Of_Threads, &hostNumberOfThreads, sizeof(uint32_t)));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(constantBitmap, hostBitmap, 8192));
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(MD5_Regenerate_Device_Number_Of_Hashes, &hostNumberOfHashes, sizeof(uint32_t)));
}

extern "C" void setMD5RegenerateNumberOfChains(uint32_t numberOfChains) {
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(MD5_Regenerate_Device_Number_Of_Chains_To_Regen, &numberOfChains, sizeof(uint32_t)));
}
...
/*
__global__ void RegenMD5ChainLen6(unsigned char *InitialPasswordArray, unsigned char *FoundPasswordArray,
    ...
    copySingleCharsetToShared(charset, MD5_Regenerate_Device_Charset_Constant);
    copyBitmap(sharedBitmap);


    // Figure out which password we are working on.
    password_index = ((blockIdx.x * blockDim.x + threadIdx.x) + (PasswordSpaceOffset * MD5_Regenerate_Device_Number_Of_Threads));


    // Return if this thread is working on something beyond the end of the password space
    if (password_index >= MD5_Regenerate_Device_Number_Of_Chains_To_Regen) {
        return;
    }
    ...
    b0 = (uint32_t) InitialArray32[0 * MD5_Regenerate_Device_Number_Of_Chains_To_Regen + password_index];
    b1 = (uint32_t) InitialArray32[1 * MD5_Regenerate_Device_Number_Of_Chains_To_Regen + password_index];
        ...
        if ((sharedBitmap[(a & 0x0000ffff) >> 3] >> (a & 0x00000007)) & 0x00000001) {
            {
                ...
                search_high = MD5_Regenerate_Device_Number_Of_Hashes;
                search_low = 0;
                search_index = 0;
                while (search_low < search_high) {
                    ...
                    if ((hash_order_a == hash_order_mem) && (search_low < MD5_Regenerate_Device_Number_Of_Hashes)) {
                            ...
                            if (c == DEVICE_Hashes_32[search_index * 4 + 2]) {
                                if (d == DEVICE_Hashes_32[search_index * 4 + 3]) {
                                    ...
                                    if (password_length >= 6) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 5] = (b1 >> 8) & 0xff;
                                            if (password_length >= 7) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 6] = deviceCharset[p6 + (MAX_CHARSET_LENGTH * 6)];
...
next:
        ...
        reduceSingleCharsetNormal(b0, b1, b2, a, b, c, d, CurrentStep, charset, charset_offset, password_length, MD5_Regenerate_Device_Table_Index);

        charset_offset++;
        if (charset_offset >= MD5_Regenerate_Device_Charset_Length) {
            charset_offset = 0;
        }
    }
    ...
    if (CurrentStep >= (MD5_Regenerate_Device_Chain_Length - 1)) {
        // Do nothing.
    }        // Else, store the b0/b1 values back to the initial array for the next loop
    else {
        InitialArray32[0 * MD5_Regenerate_Device_Number_Of_Chains_To_Regen + password_index] = b0;
        InitialArray32[1 * MD5_Regenerate_Device_Number_Of_Chains_To_Regen + password_index] = b1;
    }
}
*/


#define CREATE_MD5_REGEN_KERNEL(length) \
__global__ void RegenMD5ChainLen##length(unsigned char *InitialPasswordArray, unsigned char *FoundPasswordArray, \
        unsigned char *DeviceHashArray, uint32_t PasswordSpaceOffset, uint32_t StartChainIndex, \
        uint32_t StepsToRun, uint32_t charset_offset, unsigned char *successArray) { \
    uint32_t CurrentStep, PassCount, password_index; \
    ...
    LoadMD5RegistersFromGlobalMemory(b0,b1,b2,b3,b4,b5,b6,b7,b8,b9,b10,b11,b12,b13,b14,b15, \
        InitialArray32, MD5_Regenerate_Device_Number_Of_Chains_To_Regen, password_index, pass_length); \
    for (PassCount = 0; PassCount < StepsToRun; PassCount++) { \
        CurrentStep = PassCount + StartChainIndex; \
                        ...
                        if (b == DEVICE_Hashes_32[search_index * 4 + 1]) { \
                            if (c == DEVICE_Hashes_32[search_index * 4 + 2]) { \
                                if (d == DEVICE_Hashes_32[search_index * 4 + 3]) { \
                                    ...
                                    if (pass_length >= 2) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 1] = (b0 >> 8) & 0xff; \
                                    if (pass_length >= 3) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 2] = (b0 >> 16) & 0xff; \
                                    if (pass_length >= 4) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 3] = (b0 >> 24) & 0xff; \
                                    if (pass_length >= 5) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 4] = (b1 >> 0) & 0xff; \
                                    if (pass_length >= 6) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 5] = (b1 >> 8) & 0xff; \
                                    if (pass_length >= 7) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 6] = (b1 >> 16) & 0xff; \
                                    if (pass_length >= 8) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 7] = (b1 >> 24) & 0xff; \
                                    if (pass_length >= 9) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 8] = (b2 >> 0) & 0xff; \
                                    if (pass_length >= 10) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 9] = (b2 >> 8) & 0xff; \
                                    if (pass_length >= 11) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 10] = (b2 >> 16) & 0xff; \
                                    if (pass_length >= 12) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 11] = (b2 >> 24) & 0xff; \
                                    if (pass_length >= 13) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 12] = (b3 >> 0) & 0xff; \
                                    if (pass_length >= 14) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 13] = (b3 >> 8) & 0xff; \
                                    if (pass_length >= 15) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 14] = (b3 >> 16) & 0xff; \
                                    if (pass_length >= 16) FoundPasswordArray[search_index * MAX_PASSWORD_LENGTH + 15] = (b3 >> 24) & 0xff; \
                                    successArray[search_index] = (unsigned char) 1; \
                                } \
                            } \
                        } \
...
next: \
        reduceSingleCharsetNormal(b0, b1, b2, a, b, c, d, CurrentStep, charset, charset_offset, pass_length, MD5_Regenerate_Device_Table_Index); \
        charset_offset++; \
        if (charset_offset >= MD5_Regenerate_Device_Charset_Length) { \
            charset_offset = 0; \
        } \
...
CREATE_MD5_REGEN_KERNEL(6)
CREATE_MD5_REGEN_KERNEL(7)
CREATE_MD5_REGEN_KERNEL(8)
CREATE_MD5_REGEN_KERNEL(9)
CREATE_MD5_REGEN_KERNEL(10)


extern "C" void LaunchMD5RegenerateKernel(int PasswordLength, int CUDA_Blocks, int CUDA_Threads,
    ...
    switch (PasswordLength) {
        case 6:
            ...
            RegenMD5ChainLen6 <<< CUDA_Blocks, CUDA_Threads >>>
            ...
            break;
        case 7:
            RegenMD5ChainLen7 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
        case 8:
            RegenMD5ChainLen8 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
        case 9:
            RegenMD5ChainLen9 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
        case 10:
            RegenMD5ChainLen10 <<< CUDA_Blocks, CUDA_Threads >>>
                (InitialPasswordArray, FoundPasswordArray,
                DeviceHashArray, PasswordSpaceOffset, StartChainIndex,
                StepsToRun, charset_offset, successArray);
            break;
