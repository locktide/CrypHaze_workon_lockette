...
int main(int argc, char *argv[]) {
    ...
    struct arg_lit  *remove_source = arg_lit0(NULL, "removesource", "Removes the source file after converting successfully");
    struct arg_end  *end = arg_end(20);
    void *argtable[] = {table_files, output_directory, bits, remove_source, end};
...
#ifdef _WIN32
    _mkdir(output_directory->sval[0]);
#else
    mkdir(output_directory->sval[0], S_IRWXO | S_IXOTH | S_IRWXU | S_IRWXG);
#endif
