...
*/
        ...
        while (StartStep < this->TableHeader->getChainLength()) {
            ...
            float ref_time = cutGetTimerValue(timer);
    ...
    float total_time = cutGetTimerValue(totaltimer);
    if (this->Display) {
...
void GRTCandidateHashes::AllocatePerGPUMemory(GRTThreadRunData *data) {
    ...
    if (this->CommandLineData->GetUseZeroCopy()) {
        ...
        CUDA_SAFE_CALL(cudaMalloc((void **)&this->DEVICE_End_Hashes[data->threadID],
            this->HashLengthBytes * this->TableHeader->getChainLength() * sizeof(unsigned char)));
        CUDA_SAFE_CALL(cudaMemset(this->DEVICE_End_Hashes[data->threadID], 0,
            this->HashLengthBytes * this->TableHeader->getChainLength() * sizeof(unsigned char)));
    }
...
void GRTCandidateHashes::FreePerGPUMemory(GRTThreadRunData *data) {
    CUDA_SAFE_CALL(cudaFreeHost(this->HOST_End_Hashes[data->threadID]));
    // Only free the device memory if zero copy was NOT used
    if (!this->CommandLineData->GetUseZeroCopy()) {
        CUDA_SAFE_CALL(cudaFree(this->DEVICE_End_Hashes[data->threadID]));
    }
